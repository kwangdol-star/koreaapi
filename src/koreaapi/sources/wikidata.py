...
def parse_entity(raw: dict, entity_id: str, kind: str) -> dict:
    ...
    if ns in ("place", "medical", "university", "park", "museum", "temple", "venue", "airport", "theater", "themepark", "skiresort", "island"):  # physical locations -> coordinates (map + geo JSON-LD)
...
_ADJACENT = {  # verticals whose classes legitimately co-occur on one item (dual-typed works)
    ...
    "themepark": {"place"}, "skiresort": {"place"}, "island": {"place"},
    "place": {"museum", "temple", "venue", "airport", "theater", "themepark", "skiresort", "island"},
}
