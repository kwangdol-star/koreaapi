...
def parse_entity(raw: dict, entity_id: str, kind: str) -> dict:
    ...
    if ns in ("place", "medical", "university", "park", "museum", "temple", "venue", "airport", "theater"):  # physical locations -> coordinates (map + geo JSON-LD)
...
_ADJACENT = {  # verticals whose classes legitimately co-occur on one item (dual-typed works)
    ...
    # a stadium/arena, airport, or theater can be tagged as an attraction (Q570116 ∈ place) too — same relaxation.
    "museum": {"place"}, "temple": {"place"}, "venue": {"place"}, "airport": {"place"}, "theater": {"place"},
    "place": {"museum", "temple", "venue", "airport", "theater"},
}
