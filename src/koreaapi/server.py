...
async def get_related(entity_id: str) -> dict:
    ...
    GEO entity (place / park / temple / museum / beach …) it ALSO returns `same_region` (region-mates
    across all geo verticals) AND `nearby`: physically close verified spots ranked by great-circle km
    from verified coordinates (P625), ≤30 km — 'what's near Gyeongbokgung?'. entity_id e.g. 'artist:bts',
    'drama:squidgame', 'temple:naksansa'."""
...
def guide_resource() -> str:
    return (
        ...
        "- Don't know which tool you need? Free-text it → **ask** (\"vegetarian Korean dishes\", "
        "\"what's near Gyeongbokgung?\", \"경복궁 vs 창덕궁\") — routed to the right Answer Product and run\n"
        "- Resolve a fuzzy name / alias / external ID → the canonical entity: **get_resolve**\n"
        "- Safe to cite? how verified? → **get_verified** (Skill Score + cross/triple-verified tier); "
        "the paste-ready citation bundle → get_answer(product='evidence-pack')\n"
        "- When did a fact change (소속사 move, rename)? → **get_history** / **get_changes**\n"
        "- Officially certified by the rights-holder? → **get_certified**\n"
        "- Artist / person / agency / related graph → get_artist_status · get_person · get_agency · "
        "get_related (geo entities also return same_region + nearby, verified-coordinate km)\n"
        "- A ready decision {signal, action, score, evidence} → **get_answer** — 11 products incl. "
        "trip-plan (map-ready + walkable clusters), food-guide, compare (X vs Y), evidence-pack "
        "(catalog: koreaapi://catalog)\n\n"
